...
\newcommand{\definEnum}[1]{\stepcounter{defin}\noindent\textbf{Définition \thedefin ~:} #1}
...
\defin{Soient $G$ un groupe et $g \in G$. L'application $i_g$ : $G \rightarrow G$, $g \mapsto gxg^{-1}$ est un automorphisme de $G$ dit \textbf{intérieur}. L'ensemble des automorphismes intérieurs de $G$ forme un sous-groupe de Aut$(G)$, le groupe des automorphismes de $G$.}
...
\demo{Supposons que $D(G) \subset K$. Soient $k \in K$ et $g \in G$. $gkg^{-1} = gkg^{-1}k^{-1}k \in K$, donc $K$ est distingué. Pour montrer que $G/K$ est commutatif, il suffit de montrer que $ghK = hgK$ pour tout $g, h \in G$. Soit $k \in K$. $ghk = hgg^{-1}h^{-1}ghk = hg(g^{-1}h^{-1}gh)k \in hgK$ donc $ghK \subset hgK$. On montre l'autre inclusion de la même manière donc $ghK = hgK$. $G/K$ est alors commutatif. Réciproquement, supposons que $K$ soit distingué et que $G/K$ est commutatif. Alors, pour tout $x,y \in G$, $[xyx^{-1}y^{-1}] = [x][y][x^{-1}][y^{-1}] = [e]$ car $G/K$ est commutatif, et donc $xyx^{-1}y^{-1} \in K$. Ainsi, $D(G) \subset K$.}
...
\demo{Le premier point découle du fait que $D(H) \subset D(G)$. Le second du fait que l'ensemble des classes des éléments de $D^n(G)$ est $D^n(G/H)$.}
...
\defin{Soit $n \in \mathbb{N}^*$. On rapelle que le \textbf{groupe symétrique} d'ordre $n$, noté $S_n$ (ou $\mathcal{S}_n$) est le groupe des bijections de $\{1, ..., n\}$ dans lui même (appelées \textbf{permutations}). Il existe un unique morphisme de groupe non trivial de $S_n$ dans $\{-1, 1\}$, noté $\varepsilon$ et appelé \textbf{signature}, qui correspond à la parité du nombre de transpositions dans une décomposition d'une permutation. L'ensemble des éléments de $S_n$ dont la signature est $1$ est un sous-groupe de $S_n$ : le \textbf{sous-groupe alterné} d'ordre $n$, noté $\mathcal{A}_n$.}

\prop{Soit $n \geq 3$ et $H$ un sous-groupe distingué de $\mathcal{A}_n$ contenant un 3-cycle. Alors, $H = \mathcal{A}_n$.}
\demo{Si $n = 3$ c'est évident. Sinon, on note $\sigma = (a_1, a_2, a_3)$ le 3-cycle qu'on s'est donné. Alors, $(a_1, a_4) \circ \sigma \circ (a_1, a_4) = (a_2, a_3, a_4)$ et $(a_1, a_2) \circ \sigma \circ (a_1, a_2) = (a_1, a_3, a_2)$. On a montré que toute conjugaison d'un 3-cycle par une transposition donne un 3-cycle et on se convainc aisément qu'on peut construire tous les 3-cycles de cette manière (pour le prouver formellement il faudrait faire une disjonction de cas en fonction du support du 3-cycle que l'on souhaite construire). Or les 3-cycles engendrent $\mathcal{A}_n$. En effet, ils engendrent les doubles-transpositions (qui engendrent clairement $\mathcal{A}_n$) : $(a_1, a_2) \circ (a_2, a_3) = (a_1, a_2, a_3)$ et $(a_1, a_2) \circ (a_3, a_4) = (a_3, a_2, a_4) \circ (a_1, a_3, a_2)$.}
...
\thm{(\textit{Équation aux classes}) Soit $G$ un groupe fini. Il existe un ensemble $S$ de sous-groupes stricts de $G$ tels que : 
...
\demo{En effet, on considère l'action de groupe de $G$ sur lui-même définie par : $\forall g, h \in G$, $g \cdot h = ghg^{-1}$. Les orbites de cette action (de n'importe quelle action en fait) forment une partition de $G$. En notant $I$ un ensemble contenant un représentant de chaque orbite sauf celle de l'élément neutre, on a : $G = \omega_e \cup (\sqcup_{g \in I}~\omega_g)$ et donc 
\[\textup{Card}(G) = \textup{Card}(\omega_e) + \sum_{g \in I} \textup{Card}(\omega_g)\]
Comme l'orbite de l'élément neutre est $\mathcal{Z}(G)$, on peut réécrire cela 
\[\textup{Card}(G) = \textup{Card}(\mathcal{Z}(G)) + \sum_{g \in I} \textup{Card}(\omega_g)\]
et donc en utilisant \textbf{(P1.10)}
\[\textup{Card}(G) = \textup{Card}(\mathcal{Z}(G)) + \sum_{g \in I} \frac{\textup{Card}(G)}{\textup{Card}(S_g)}\]}
...
\defin{Soit $(A, +, \times)$ un anneau commutatif et $I$ un idéal de $A$. On dit que $I$ est \textbf{principal} lorsqu'il existe $x \in A$ tel que $I = \{xa$, $a \in A\}$ (que l'on note $(x)$ ou $xA$). \\
...
\defin{Soient $A_{1}$ et $A_{2}$ des anneaux et $f : A_{1} \rightarrow A_{2}$ une application. On dit que $f$ est un \textbf{morphisme} d'anneaux lorsque :
	...
	\item $f(1) = 1$
	\end{enumerate}
On vérifie que le \textbf{noyau} (\textit{i.e., $\{x \in A_{1},$ $f(x) = 0\}$}) d'un morphisme d'anneau $f : A_{1} \rightarrow A_{2}$ est un idéal de $A_{1}$.}


\defin{Soit $A$ un anneau. L'application $f : \mathbb{Z} \rightarrow A$, $k \mapsto k \cdot 1 = 1 + ... + 1~(k \textup{ fois})$ est un morphisme d'anneau et son noyau, étant un idéal de $\mathbb{Z}$, est de la forme $c\mathbb{Z}$ pour un certain $c \in \mathbb{N}$. L'entier $c$ est la \textbf{caractéristique} de $A$.}
...
\demo{Soit $x \in$ Ker$(f)$. Si $x \neq 0$, $1 = f(xx^{-1}) = f(x)f(x^{-1}) = 0f(x^{-1}) = 0$, ce qui est absurde. Bref, Ker$(f) = \{0\}$.}


\defin{Soit $\mathbb{K}$ un corps et $\mathds{k} \subseteq \mathbb{K}$. On dit que $\mathds{k}$ est un \textbf{sous-corps} de $\mathbb{K}$ lorsque c'est un corps pour les lois induites par $\mathbb{K}$. Si $\mathds{k}$ est un sous-corps de $\mathbb{K}$, on dit que $\mathbb{K}$ est une \textbf{extension} de corps de $\mathds{k}$. Plus généralement, si $\mathds{k}$ est un corps pas forcément contenu dans $\mathbb{K}$ mais qu'il existe un morphisme de $\mathds{k}$ dans $\mathbb{K}$, on dira aussi que $\mathbb{K}$ est une extension de corps de $\mathds{k}$, car, ce morphisme étant injectif (\textbf{1.6}), $\mathds{k}$ est isomorphe à un sous-corps de $\mathbb{K}$.}
...
\defin{Soit $A$ un anneau commutatif. Un polynôme $P \in A[X]$ est dit \textbf{irréductible} sur $A$ \textit{(ou dans A[X])} s'il n'est ni inversible, ni produit de deux polynômes non inversibles.}
...
\defin{Soit $\mathds{k}$ un corps, $\mathbb{K}$ une extension de $\mathds{k}$ et $P \in \mathds{k}[X]$ de degré $\geq 1$. On dit que $x \in \mathbb{K}$ est une \textbf{racine} de $P$ lorsque $P(x) = 0$. On remarque que $x \in \mathbb{K}$ est racine de $P$ si et seulement si $X - x$ divise $P$. On en déduit que le nombre de racines (comptées avec leur multiplicité, \textit{i.e., l'unique entier $n \in \mathbb{N}^*$ tel que $(X - x)^n$ divise $P$ et $(X-x)^{n+1}$ ne divise pas $P$}) de $P$ est inférieur ou égal au degré de $P$.}


\defin{Un polynôme $P \in \mathbb{K}[X]$ de degré $n \geq 1$ est dit \textbf{scindé} lorsqu'il peut s'écrire comme produit de $n$ polynômes de degré 1. Si $\mathbb{K}$ est un corps, $P$ est scindé si, et seulement s'il a $n$ racines (comptées avec leur multiplicité).}
...
\demo{C'est exactement la même démonstration que pour \textbf{(P2.2)} mais adaptée aux polynômes.}
\newline
...
\demo{En effet, $[X]$ est racine de $P$ dans $\mathbb{K}[X]/(P)$, qui est bien une extension de $\mathbb{K}$ car $x \mapsto [x]$ est un morphisme de $\mathbb{K}$ dans $\mathbb{K}[X]/(P)$. L'unicité découle du fait que si $\mathbb{L}$ est un corps de rupture de $P$ et que $a \in \mathbb{L}$ est une racine de $P$, alors $\mathbb{K}[X]/(P) \rightarrow \mathbb{L},~[A] \mapsto A(a)$ est un isomorphisme car $P$ est associé au polynôme minimal de $a$ (voir \textbf{(P4.3)}).}
